...
const showComfirmDeleteDiscountModel = ref(false);

const { hasPermission } = usePermission();

const ComfimDeleteDiscount = (id) => {
  showComfirmDeleteDiscountModel.value = true;
  discountId.value = id;
};

const closeModal = () => {
  showComfirmDeleteDiscountModel.value = false;
};

const DeleteDiscount = () => {
  form.delete(route("discounts.destroy", discountId.value), {
        ...
        <Table class="w-full bg-white dark:bg-slate-300">
          ...
          <template #default>
            <TableRow
              ...
              class="border-b"
            >
              <TableDataCell>{{ Discount.id }}</TableDataCell>
              <TableDataCell>
                <!-- <img src="`${{ Discount.image }}`" alt=""> -->
                <img
                  v-if="Discount.image.length > 0"
              ...
              <TableDataCell>{{ Discount.title }}</TableDataCell>
              <TableDataCell>{{ Discount.percentage }}</TableDataCell>
              <TableDataCell>{{ Discount.description }}</TableDataCell>
              <TableDataCell>
                <span class="text-yellow-500 flex">
                  ...
                  <button
                    ...
                    @click="ComfimDeleteDiscount(Discount.id)"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                  ...
                  <Modal
                    :show="showComfirmDeleteDiscountModel"
                    @click="closeModal"
                  >
                  ...
                  <!-- |
                  <button
                    @click="DeleteDiscount(Category.id)"
                  >
