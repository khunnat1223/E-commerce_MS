...
const showComfirmDeleteCategoryModel= ref(false);

const {hasPermission}=usePermission();


const ComfimDeleteCategory =(id)=>{
showComfirmDeleteCategoryModel.value=true;
categoryID.value= id;

}

const closeModal = () => {
  showComfirmDeleteCategoryModel.value=false;
}

const deleteCategory = () => {
form.delete(route('categorys.destroy', categoryID.value),{
    ...
    <div class="px-8 w-full">
      <div class="flex justify-between pb-4">
        ...
        <div class="text-md font-sans cursor-pointer flex">
          ...
          >
            ...
            <span class="m-1">
              <Link :href="route('categorys.index')" :active="false">
      ...
      <div class="shadow overflow-hidden rounded border-b border-gray-200">
        <Table class="w-full bg-white dark:bg-slate-300">
          ...
          <template #default>
            <TableRow v-for="Category in categorys" :key="Category.id" class="border-b">
              <TableDataCell>{{ Category.id }}</TableDataCell>
              <TableDataCell>{{ Category.category }}</TableDataCell>
              <TableDataCell>{{ Category.description }}</TableDataCell>
              <TableDataCell>
                <span class="text-yellow-500 flex">
                  <Link
                    v-if="hasPermission('Update Product')"
                   :href="route('categorys.edit', Category.id)">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                  ...
                  v-if="hasPermission('Delete Product')"
                    @click="ComfimDeleteCategory(Category.id)"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                  ...
                  <Modal :show="showComfirmDeleteCategoryModel" @click="closeModal">
                      <div class="relative p-4 text-center  rounded-lg">
                        ...
                        <p class="mb-4 text-gray-500 dark:text-gray-300">{{ $t('comfimdelete') }} {{Category.id}}</p>

                        <DangerButton @click="deleteCategory">{{ $t('delete') }}</DangerButton>
