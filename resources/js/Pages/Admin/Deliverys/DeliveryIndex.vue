...
defineProps(["deliverys"]);
const form = useForm({});
const DeliveryID = ref("");

const showComfirmDeleteDeliveryModel= ref(false);

const {hasPermission}=usePermission();


const ComfimDeleteDelivery =(id)=>{
showComfirmDeleteDeliveryModel.value=true;
DeliveryID.value= id;

}

const closeModal = () => {
showComfirmDeleteDeliveryModel.value=false;
}

const deleteDelivery = () => {
form.delete(route('deliverys.destroy', DeliveryID.value),{
      ...
      <div class="flex justify-between pb-4">
        ...
        <div class="text-md font-sans cursor-pointer flex">
          ...
          >
            ...
            <span class="m-1">
              <Link :href="route('deliverys.index')" :active="false">
                <span class="font-sans">{{ $t("Delivery") }}</span>
              </Link>
            </span>
          </div>
        </div>
        ...
        <Table class="w-full bg-white dark:bg-slate-300">
          <template #header>
            <TableRow>
              ...
              <TableHeaderCell>{{ $t("Delivery") }}</TableHeaderCell>
              <TableHeaderCell>{{ $t("Description") }}</TableHeaderCell>
              <TableHeaderCell>{{ $t("Action") }}</TableHeaderCell>
            </TableRow>
          </template>
          <template #default>
            <TableRow v-for="Delivery in deliverys" :key="Delivery.id" class="border-b">
              <TableDataCell>{{ Delivery.id }}</TableDataCell>
              <TableDataCell>{{ Delivery.company }}</TableDataCell>
              <TableDataCell>{{ Delivery.description }}</TableDataCell>
              <TableDataCell>
                <span class="text-yellow-500 flex">
                  <Link
                    v-if="hasPermission('Update Product')"
                   :href="route('deliverys.edit', Delivery.id)">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                  ...
                  v-if="hasPermission('Delete Product')"
                    @click="ComfimDeleteDelivery(Delivery.id)"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                  ...
                  <Modal :show="showComfirmDeleteDeliveryModel" @click="closeModal">
                      <div class="relative p-4 text-center  rounded-lg">
                        ...
                        <DangerButton @click="deleteDelivery">{{ $t('delete') }}</DangerButton>
