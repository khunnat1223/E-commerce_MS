...
<template>
  <header class="h-16 bg-gray-800 -z-0">
    <div
      class="flex flex-wrap pt-2 items-center justify-between px-4 mx-auto max-w-screen-2xl md:px-10"
    >
      <!-- logo - start -->
      <a
        href="/"
        class="inline-flex  items-center gap-2.5 text-xl text-white md:text-3xl mt-2"
        aria-label="logo"
      >
      ...
      <div v-if="canLogin" class="flex gap-0 lg:flex 2xl:ml-16 -mt-3">
        ...
        >
          <svg
            ...
            stroke-width="1.5"
            stroke="currentColor"
            class="size-6 text-white dark:text-yellow-400"
          >
            <path
              stroke-linecap="round"
              stroke-linejoin="round"
        ...
        >
          ...
          <ul class="py-2" aria-labelledby="user-menu-button">
            <li v-if="auth.user.name">
              <Link
                :href="route('Userprofile.edit', auth.user.id)"
